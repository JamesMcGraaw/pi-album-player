...
1. Run the test script and tap the tag:
   ```bash
   sudo python3 ~/pn532-nfc-hat-code/Pn532-nfc-hat-code/raspberrypi/python/example_get_uid.py
   ```
2. Note the UID — e.g. `['0xb7', '0xa8', '0x87', '0x3']` → `b7a88703`
3. Get the Spotify URI — right-click album in Spotify desktop app → Share → hold **Alt** → Copy Spotify URI
4. Add to `TAG_MAP` in `~/nfc_player_config.py`:
   ```python
